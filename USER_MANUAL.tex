...
\begin{itemize}
    ...
    \item Each field gets a column named \texttt{\{SurveyType\}\_\{FieldName\}}, or its mapped friendly name if a field mapping is configured
    ...
    \item Example columns: \texttt{demographics\_Age}, \texttt{nasa1\_Mental\_Demand}, \texttt{Gender} (if \texttt{Check1} is mapped to ``Gender'')
\end{itemize}
...
\begin{itemize}
    ...
    \item Field mapping (system field names $\rightarrow$ user-friendly column names)
\end{itemize}

\subsection{Field Mapping}

Field mapping defines user-friendly names for system field identifiers (e.g., mapping \texttt{demographics\_Check1} to ``Gender''). These names appear in the prefill dialog and as column headers in the masterfile.

\begin{enumerate}
    \item Go to \textbf{File $\rightarrow$ Save/Edit Field Mapping}
    \item A table shows all fields (\textbf{Field} column, read-only) and their current names (\textbf{Name} column, editable)
    \item Edit the Name column to set user-friendly labels
    \item Click \textbf{Save} -- the mapping is persisted to the active configuration
\end{enumerate}

\begin{itemize}
    \item Load a field mapping from another configuration via \textbf{File $\rightarrow$ Load Field Mapping}
    \item Delete a mapping via \textbf{File $\rightarrow$ Delete Field Mapping}
    \item The mapping is automatically applied when extracting data and during the prefill dialog
\end{itemize}
