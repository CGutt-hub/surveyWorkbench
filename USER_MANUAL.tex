...
\hypersetup{
    ...
    linkcolor=blue,
    urlcolor=blue,
    pdftitle={Survey Workbench v2.1 - User Manual},
    pdfauthor={}
}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Survey Workbench v2.1}
\fancyhead[R]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

\title{\textbf{Survey Workbench v2.1}\\[0.5em]\Large User Manual}
\author{}
\date{June 2026}
...
\vfill
\begin{center}
\textit{Version 2.1 -- Internal Use Only}
\end{center}
\newpage
...
The Survey Workbench is a comprehensive tool for managing participant folders and extracting survey data from questionnaires. Version 2.1 introduces field mapping auto-generation from form templates, non-participant-specific column naming for multi-participant files, and streamlined configuration management.
...
When you first launch Survey Workbench v2.1, you'll see the main window with two primary sections:
...
\begin{enumerate}
    ...
    \item Open the \textbf{Prefill Dialog} (see below)
\end{enumerate}

\subsubsection{Step 6: Prefill Dialog}

After generating, a table appears showing all PDF form fields grouped for readability:

\begin{itemize}
    \item \textbf{Field} -- the system field name (e.g., \texttt{Check1}, \texttt{Text2})
    \item \textbf{Name} -- the user-friendly label from the field mapping (e.g., ``Gender'')
    \item \textbf{Data} -- an editable value to prefill into the PDF before handover
\end{itemize}

\paragraph{Checkbox grouping:}
Checkboxes sharing a base name (e.g., \texttt{Check1\_1}, \texttt{Check1\_2}, \texttt{Check1\_3}) are shown as a single row \texttt{Check1}. Enter the option number to prefill (e.g., \texttt{1} to check option 1, leaving the rest unchecked). The system translates this to \texttt{/Yes} and \texttt{/Off} PDF values automatically.

\paragraph{Name column:}
Editing the Name column here updates the field mapping for the active configuration, which is also used as the column header in the masterfile.
...
\begin{itemize}
    ...
    \item Each field gets a column named with the field identifier (e.g., \texttt{Age}, \texttt{Mental\_Demand}), or its mapped friendly name if a field mapping is configured
    \item Column names are \textit{non-participant-specific}, allowing multiple participants to be added to the same masterfile
    \item Checkbox groups (e.g., \texttt{Check1\_1}, \texttt{Check1\_2}) are collapsed into a single column (e.g., \texttt{Check1}) containing the option number that was checked
    \item Example columns: \texttt{Age}, \texttt{Mental\_Demand}, \texttt{Gender} (if \texttt{Check1} is mapped to ``Gender'')
\end{itemize}
...
\begin{itemize}
    \item Target folder path (for generation)
    ...
    \item Masterfile path (for extraction)
    \item \textbf{Field mapping} (system field identifiers $\rightarrow$ user-friendly column names) -- auto-generated from templates and editable via \textit{Save/Edit Field Mapping}
\end{itemize}

\subsection{Field Mapping}

Field mapping defines user-friendly names for system field identifiers (e.g., mapping \texttt{Check1} to ``Gender''). These names appear in the prefill dialog and as column headers in the masterfile.

\subsubsection{Auto-Generation}

When you click \textbf{File $\rightarrow$ Save/Edit Field Mapping}, the system automatically scans your configured form templates and generates an initial field mapping. This ensures the mapping always matches the actual form fields in your templates.

\subsubsection{Editing the Mapping}

\begin{enumerate}
    \item Go to \textbf{File $\rightarrow$ Save/Edit Field Mapping}
    \item A table shows all fields (\textbf{Field} column, read-only) auto-detected from your form templates
    \item Edit the \textbf{Name} column to set user-friendly labels
    \item Click \textbf{Save} -- the mapping is saved to the active configuration
\end{enumerate}

\subsubsection{Persistence}

\begin{itemize}
    \item The mapping is automatically saved with your project configuration
    \item When you load a configuration, its field mapping loads automatically
    \item The mapping is applied when extracting data and during the prefill dialog
    \item No separate load/delete operations needed -- field mappings are integral to the project configuration
\end{itemize}
...
\subsection{Form Field Naming Convention}

For consistent CSV extraction, name PDF form fields using this pattern:
\texttt{NameX(\_N)(\_n)}

\begin{itemize}
    \item \texttt{Name}: Use \texttt{Text} for text fields and \texttt{Check} for checkboxes.
    \item \texttt{X}: Field group number. Fields that belong to the same question share the same \texttt{X}.
    \item \texttt{\_N}: Checkbox option number (only for \texttt{Check} fields).
    \item For binary checkboxes, use \texttt{\_1} for Yes and \texttt{\_0} for No.
    \item For multi-option checkboxes, number options in ascending order: \texttt{\_0}, \texttt{\_1}, \texttt{\_2}, etc.
    \item \texttt{\_n}: Append this suffix for fields that should not be preferred for extraction.
\end{itemize}

\textbf{Examples:} \texttt{Text1}, \texttt{Check4\_1}, \texttt{Check4\_0}, \texttt{Text11\_n}

\section{Technical Information}

\subsection{Version History}

\subsubsection{v2.1 (June 2026)}
\begin{itemize}
    \item Field mapping auto-generated from form templates
    \item Non-participant-specific column names (support multiple participants per masterfile)
    \item Streamlined field mapping workflow (integrated with project configuration)
    \item Removed redundant load/delete field mapping operations
    \item Enhanced type hints and code quality
    \item Updated documentation for v2.1 features
\end{itemize}
...
\textit{Survey Workbench v2.1 -- User Manual}\\[0.5em]
\textit{June 2026}
\end{center}
