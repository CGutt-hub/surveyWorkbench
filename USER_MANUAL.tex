...
\subsection{N -- Checkbox Option Number (Check only)}

\texttt{N} is only used for \texttt{Check} fields and identifies the specific checkbox within its group:

\begin{itemize}
    \item \textbf{Binary (yes/no):} Use \texttt{1} for \textit{yes} and \texttt{0} for \textit{no}.
    \item \textbf{Multiple choice:} Number each option sequentially (1, 2, 3, \ldots), where each number is higher than the one before.
\end{itemize}

\subsection{\_ -- Not-Extracted Prefix}

Prefixing a field name with an underscore (\texttt{\_}) marks the field as \textbf{excluded from extraction}. Survey Workbench will skip these fields entirely when processing the questionnaire data. The underscore is placed before the \texttt{Name} part of the field name.
...
\begin{itemize}
    ...
    \item \textbf{Checkbox group} (\texttt{CheckX\_N}): All options in the group share one output column. The value written is the \textbf{N} of whichever option is selected. If nothing is selected the value is empty. Examples: \texttt{Check2\_0} selected $\Rightarrow$ \texttt{0}; \texttt{Check1\_2} selected $\Rightarrow$ \texttt{2}; binary pair \texttt{Check3\_0}/\texttt{Check3\_1} with \texttt{Check3\_1} ticked $\Rightarrow$ \texttt{1}.
    \item \textbf{Not-extracted field} (\texttt{\_NameX(\_N)}): The field is ignored and does not appear in the masterfile.
\end{itemize}
...
\texttt{\_Text2}    & Free-text field, group 2 -- not extracted \\
...
\texttt{Check4\_3}  & Checkbox, group 4, option 3 (multiple choice) \\
\texttt{\_Check5\_1} & Checkbox, group 5 -- not extracted \\
\end{tabular}
